...
def dashboard_view(request):
    ...
    """
    total_income = (
        FinancialTransaction.objects.filter(type="income").aggregate(Sum("amount"))[
            "amount__sum"
        ]
        or 0
    )
    total_expense = (
        FinancialTransaction.objects.filter(type="expense").aggregate(Sum("amount"))[
            "amount__sum"
        ]
        or 0
    )
    ...
    upcoming_programmes = Programme.objects.filter(
        start_time__gte=timezone.now()
    ).order_by("start_time")[:5]

    recent_transactions = FinancialTransaction.objects.all()[
        :5
    ]  # Display last 5 transactions

    # Get recent incomplete to-do items for all users
    recent_todo_items = TodoItem.objects.filter(completed=False).order_by("due_date", "priority")[:5]

    # Get recent commercials
    recent_commercials = Commercial.objects.filter(
        end_date__gte=timezone.now()
    ).order_by("start_date")[
        :5
    ]  # Commercials that are active or upcoming
...
def programme_create_view(request):
    ...
    return render(
        request,
        "ggfm_dbs/programme_form.html",
        {"form": form, "title": "Add New Programme"},
    )
...
def programme_update_view(request, pk):
    ...
    return render(
        request,
        "ggfm_dbs/programme_form.html",
        {"form": form, "title": "Update Programme"},
    )
...
def programme_delete_view(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("programme_list")
    return render(
        request, "ggfm_dbs/confirm_delete_programme.html", {"programme": programme}
    )
...
def financial_transaction_create_view(request):
    ...
    return render(
        request,
        "ggfm_dbs/financial_transaction_form.html",
        {"form": form, "title": "Add New Financial Transaction"},
    )
...
def financial_transaction_update_view(request, pk):
    ...
    return render(
        request,
        "ggfm_dbs/financial_transaction_form.html",
        {"form": form, "title": "Update Financial Transaction"},
    )
...
def financial_transaction_delete_view(request, pk):
    ...
    return render(
        request,
        "ggfm_dbs/confirm_delete_financial_transaction.html",
...
def todo_list_view(request):
    """
    Displays a list of all to-do items for all users.
    """
    todo_items = TodoItem.objects.all().order_by(
...
def todo_detail_view(request, pk):
    ...
    """
    todo_item = get_object_or_404(TodoItem, pk=pk)  # Remove user-specific check
...
def todo_create_view(request):
    ...
    return render(
        request,
        "ggfm_dbs/todo_form.html",
        {"form": form, "title": "Add New To-Do Item"},
    )
...
def todo_detail_view(request, pk):
    ...
    todo_item = get_object_or_404(
        TodoItem, pk=pk, created_by=request.user
...
def todo_update_view(request, pk):
    ...
    return render(
        request, "ggfm_dbs/todo_form.html", {"form": form, "title": "Update To-Do Item"}
    )
...
def todo_delete_view(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("todo_list")
    return render(
        request, "ggfm_dbs/confirm_delete_todo.html", {"todo_item": todo_item}
    )
...
def commercial_create_view(request):
    ...
    return render(
        request,
        "ggfm_dbs/commercial_form.html",
        {"form": form, "title": "Add New Commercial"},
    )
...
def commercial_update_view(request, pk):
    ...
    return render(
        request,
        "ggfm_dbs/commercial_form.html",
        {"form": form, "title": "Update Commercial"},
    )
...
def commercial_delete_view(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("commercial_list")
    return render(
        request, "ggfm_dbs/confirm_delete_commercial.html", {"commercial": commercial}
    )
